...
> Istio可以轻松的创建网络，在网络部署服务，就可以具有负载平衡，服务到服务认证，监控等等功能，而不需要改动任何服务代码。
...
图中应用作为服务的发起方，只需要用最简单的方式将请求发送给本地的linkerd实例，然后linkerd实例会进行后续的操作，如服务发现，负载均衡，最后将请求转发给目标服务。

当有多个服务串行调用时，他们之间的服务网格如下图所示：

![](images/diagram-sidecar-deployment.png)

服务网格的细节我们今天不展开, 详细内容大家可以参考其他的资料和网上分享。
...
- 平台支持。Istio旨在在各种环境中运行，包括Kubernetes，Mesos等各种环境。最初专注于Kubernetes，但很快会支持其他环境。
...
Google 和 IBM 相信不需要介绍了, 在 istio 项目中这两个公司是绝对主力. 举个例子,下图是 istio Working Group的成员列表:
...
Lyft 的贡献主要集中在 Envoy 代理, 这是 Lyft 开源的服务网格, 基于C++. 据说 Envoy 在Lyft可以管理超过100个服务, 跨越10000个虚拟机，每秒处理2百万请求。本周最新消息，Envoy刚刚加入CNCF，成为该基金会的第十一个项目。
